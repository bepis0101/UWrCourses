...
def fully_random_experiment():
  init_figurant = [i for i in range(11, 15)]
...
cards_figurant = [Card(i, j) for i in range(4) for j in range(11, 15)]
cards_blotkarz = [Card(i, j) for i in range(4) for j in range(2, 11)]

figurant_combs = list(combinations(cards_figurant, 5))
blotkarz_combs = list(combinations(cards_blotkarz, 5))

figurant = Counter(list(map(power, figurant_combs)))
blotkarz = Counter(list(map(power, blotkarz_combs)))
...
for i in range(1, 10):
  if i == 1:
    figurant_wins += len(figurant_combs)*blotkarz[i]
    continue
  # liczymy ile bedzie ukladow blotkarza > od figuranta oraz ile bedzie ukladow figuranta >= od blotkarza
  for j in range(1, i):
    blotkarz_wins += figurant[j]*blotkarz[i]
  for j in range(1, i+1):
    figurant_wins += figurant[i]*blotkarz[j]

print(blotkarz_wins/(figurant_wins+blotkarz_wins))