...
(define-type Exp
  ...
  (lamE [ls : (Listof Symbol)] [e : Exp])
  (appE [e : Exp] [ls : (Listof Exp)]))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
   ...
   [(lamE ls b)
    (eval-lam ls b env)]
    [(appE e ls)
     (apply (eval e env) ls env)]))

(define (apply [v : Value] [ls : (Listof Exp)] [env : Env]) : Value
  (type-case Value v
    [(funV f)
     (type-case (Listof Exp) ls
       [empty (error 'apply "too few arguments")]
       [(cons e rest-ls)
        (apply (f (eval e env)) rest-ls env)])]  
    [else v]))

(define (eval-lam [ls : (Listof Symbol)] [e : Exp] [env : Env]) : Value
  (type-case (Listof Symbol) ls
    [empty (eval e env)]
    [(cons x xs)
     (funV (λ (v) (eval-lam xs e (extend-env env x v))))]))
