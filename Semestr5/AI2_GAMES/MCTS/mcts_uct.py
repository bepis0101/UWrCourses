...
def tree_policy(node):
  while not node.state.is_terminal():
    if len(node.children) < 4:
      ...
      while move in node.done_moves:
        move = choice(MOVES.keys())
      node.done_moves.append(move)
      child = [Node(deepcopy(node.state)), move]
      node.children.append(child)
      return child
    else:
      node = best_uct(node)

def best_uct(node, c=2):
  return max(node.children, key=lambda x: x[0].reward/x[0].visits + c * (2 * log(node.visits) / x[0].visits)**0.5)

def mcts_search(root):
  node = Node(root)
  start = datetime.now()
  while calc_miliseconds(start, datetime.now()) < 100:
    leaf = tree_policy(node)
    value = default_policy(leaf)
    backup(leaf, value)
  return best_child(node).state
